...
# Reuse existing nodebuilder-dashy container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-dashy; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-dashy -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/lissy93/dashy:latest" \
    "${container}"
# Commit the image
